...
class ReleaseMatrixTest(unittest.TestCase):
    ...
    def test_fork_version_is_preserved_in_plan_and_archive(self):
        with patch.dict(os.environ, {'GITHUB_OUTPUT': 'outputs'}), patch.object(subprocess, 'check_output', return_value='a' * 40 + '\n'):
            release.plan(argparse.Namespace(ref='v0.2.8.1', dry_run=False, simple=True))
    ...
    def test_publication_rejects_non_fork_versions(self):
        for tag in ('v0.2.8', 'v0.2.8.0', 'v0.2.8.1-rc1', 'v00.2.8.1'):
            with self.subTest(tag=tag), patch.object(subprocess, 'check_output', return_value='a' * 40 + '\n'):
                with self.assertRaisesRegex(ValueError, 'version tag'):
                    release.plan(argparse.Namespace(ref=tag, dry_run=False, simple=True))
