...
VERSION_RE = re.compile(r'\d+\.\d+\.\d+(?:\.[1-9]\d*)?(?:-[0-9A-Za-z.-]+)?')
FORK_VERSION_RE = re.compile(r'(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.[1-9]\d*')
...
def plan(args):
    ...
    if args.dry_run:
        ...
        if not tag.startswith('v') or not FORK_VERSION_RE.fullmatch(version):
            raise ValueError('publishing requires a vX.Y.Z.N fork release version tag')
