...
func TestSettingsCodexTicketAccountSelection(t *testing.T) {
	key := service.SettingKeyOpenAICodexTicketAccountIDs
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{key: "[363]"})
	for _, test := range []struct {
		body   map[string]any
		stored string
	}{
		{map[string]any{key: []int64{42, 41, 42}}, "[41,42]"},
		{map[string]any{"site_name": "unrelated"}, "[41,42]"},
		{map[string]any{key: []int64{}}, "[]"},
	} {
		rec := doUpdateSettings(t, h, test.body, nil)
		require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
		require.Equal(t, test.stored, repo.values[key])
		require.Contains(t, rec.Body.String(), `"openai_codex_ticket_account_ids":`+test.stored)
	}
	for _, ids := range [][]int64{{0}, {-1}} {
		rec := doUpdateSettings(t, h, map[string]any{key: ids}, nil)
