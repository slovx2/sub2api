...
func (h *Handler) GetConfig(c *gin.Context) {
	...
	if h.service == nil || h.service.cfg == nil {
		log.Printf("payment config requested: enabled=%t min=%d max=%d step=%d cfg=nil", cfg.Enabled, cfg.MinAmount, cfg.MaxAmount, cfg.Step)
		response.Success(c, cfg)
		return
	}
	raw := h.service.cfg
	log.Printf(
		"payment config requested: enabled=%t min=%d max=%d step=%d public_base_url=%t merchant_id=%d private_key_len=%d public_key_len=%d",
		cfg.Enabled,
		cfg.MinAmount,
		cfg.MaxAmount,
		cfg.Step,
