...
func TestSetModelRateLimitByModelName_UsesScopeKey(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "unknown model should fail",
			modelName:        "gpt-5",
			expectedModelKey: "",
...
func TestUpdateAccountModelRateLimitInCache_UpdatesExtraAndCallsCache(t *testing.T) {
	...
	modelKey := antigravityRateLimitScopeClaude
	resetAt := time.Now().Add(30 * time.Second)
...
func TestUpdateAccountModelRateLimitInCache_NilSchedulerSnapshot(t *testing.T) {
	...
	// 不应 panic
	svc.updateAccountModelRateLimitInCache(context.Background(), account, antigravityRateLimitScopeClaude, time.Now().Add(30*time.Second))
...
func TestUpdateAccountModelRateLimitInCache_PreservesExistingExtra(t *testing.T) {
	...
	require.NotNil(t, limits[antigravityRateLimitScopeGeminiText])
	require.NotNil(t, limits[antigravityRateLimitScopeClaude])
}
